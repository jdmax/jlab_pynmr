...
def main():
    ...
    RegSets.append((10).to_bytes(2, 'little'))  # GenSetTime (or trig rate 1 count=20us)
    RegSets.append((2).to_bytes(2, 'little'))  # NumOfSamToAve or pulse width

    RegSets.append((128).to_bytes(2, 'little'))  # Number of sweeps
    RegSets.append((64).to_bytes(2, 'little'))  # Sweeps per chunk

    RegSets.append(ADCConfig.to_bytes(2, 'little'))
    RegSets.append(dac_value.to_bytes(2, 'little'))
    RegSets.append((4).to_bytes(1, 'little'))
